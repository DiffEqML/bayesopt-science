...
from sympy import *
from src.models.initialization import init_cond
...
def get_density(T, P, MW, m, sigma, eps_k, kappa_AB, epsAB_k, Nassoc, x_vec, eta_guess):
	S = len(x_vec)
	M = int(sum(Nassoc))
	...
	error_glob.append(init_cond(T, P, MW, m, sigma, eps_k, kappa_AB, epsAB_k, Nassoc, x_vec, 0.9*eta_guess))
	eta_glob.append(init_cond.eta)
	eta_glob.append(eta_guess)

	#############################################################
	################# BEGINNING OF CODE #########################
	#############################################################
	while abs(error_glob[main_itr]) > tol and main_itr < max_iter:
		if main_itr == max_iter - 10:
			print('Getting too close to iteration limit. Exiting...')
			break
		main_itr += 1

		eta = eta_glob[main_itr]
		...
		ddidT = np.zeros(S)
		for i in range(S):
			ddidT[i] = sigma[i]*(3.*eps_k[i]/T**2)*(-0.12*np.exp(-3*eps_k[i]/T))

		zeta1pT = 0
		zeta2pT = 0
		zeta3pT = 0
		for i in range(S):
			zeta1pT += (x_vec[i]*m[i]*1*ddidT[i]*d[i]**(1-1))*(rho*np.pi/6) 
			zeta2pT += (x_vec[i]*m[i]*2*ddidT[i]*d[i]**(2-1))*(rho*np.pi/6) 
			zeta3pT += (x_vec[i]*m[i]*3*ddidT[i]*d[i]**(3-1))*(rho*np.pi/6) 

		dghs_dT = np.zeros(S)
		for i in range(S):
			dghs_dT[i] = zeta3pT/(1 - zeta3)**2 + 0.5*ddidT[i]*3*zeta2/(1 - zeta3)**2 + \
				0.5*d[i]*(3*zeta2pT/(1 - zeta3)**2 + 6*zeta2*zeta3pT/(1 - zeta3)**3) + \
				0.5*d[i]*ddidT[i]*2*zeta2**2/(1 - zeta3)**3 + \
				(0.5*d[i])**2*(4*zeta2*zeta2pT/(1 - zeta3)**3 + 6*zeta2**2*zeta3pT/(1 - zeta3)**4)

		temp_hs = 0
		for i in range(S):
			temp_hs += x_vec[i]*(m[i] - 1)*dghs_dT[i]/g_hs[i,i]

		dahsdT = (3*(zeta1pT*zeta2 + zeta1*zeta2pT)/(1 - zeta3) + \
			3*zeta1*zeta2*zeta3pT/(1 - zeta3)**2 + 3*zeta2**2*zeta2pT/(zeta3*(1 - zeta3)**2) + \
			zeta2**3*zeta3pT*(3*zeta3 - 1)/(zeta3**2*(1 - zeta3)**3) + \
			((3*zeta2**2*zeta2pT*zeta3 - 2*zeta2**3*zeta3pT)/zeta3**3)*np.log(1 - zeta3) + \
			(zeta0 - zeta2**3/zeta3**2)*(zeta3pT/(1 - zeta3)))/zeta0
		dahcdT = mbar*dahsdT - temp_hs
		...
		I2pT = 0
		for i in range(7):
			I1pT += a[i]*i*zeta3pT*eta**(i-1)
			I2pT += b[i]*i*zeta3pT*eta**(i-1)

		C1pT = zeta3pT*C2

		dadispdT = -2*np.pi*rho*(I1pT - I1/T)*term1 - \
			np.pi*rho*mbar*(C1pT*I2 + C1*I2pT - 2*C1*I2/T)*term2
		...
		if np.dot(x_vec,Nassoc) != 0:
			...
			while norm(resX) > tol_assoc and itr < max_itr:
				itr += 1
				if itr % 100 == 0:
					print("Inner Iteration (1):",itr)
				if itr == max_itr - 5:
					raise SystemError('Getting too close to iteration limit in Zassoc (1). Increase number of assoc iterations!')
				for i in range(M):
					temp_res = 0
					indx = 0
					for j in range(S):
			...
			itr = 0
			while norm(resdXA) > tol_assoc and itr < max_itr:
				itr += 1
				if itr % 100 == 0:
					print("Inner Iteration (2):",itr)
				if itr == max_itr - 5:
					raise SystemError('Getting too close to iteration limit in Zassoc (2). Increase number of assoc iterations!')
				for i in range(S):
					if i == 0:
						indx1 = 0
					if i == 1:
					...
					for j in range(M):
						...
						dXA_drho[i,j] = dXA_drho[i,j] - 0.1*resdXA[i,j] 
		...
		daresdT = dahcdT + dadispdT + daassocdT
		
		hresRT = -T*daresdT + (Z - 1)
		#############################################################
		############### UPDATING LOOP PARAMETERS ####################
		#############################################################
		Pnew = Z*kb*T*rho*(10**10)**3
		error = (Pnew - P)/P
		error_glob.append(error)

		residual = (eta_glob[main_itr] - eta_glob[main_itr-1])*\
			error_glob[main_itr]/(error_glob[main_itr] - error_glob[main_itr-1])
		new_eta = eta_glob[main_itr] - residual


		if new_eta < 0 or new_eta > 0.7405:
			if eta_guess == 0.5:
				eta_glob.append(10**(-10))
			if eta_guess == 10**(-10):
				eta_glob.append(0.5)	
		else:
			eta_glob.append(new_eta)

	rho_hat = (6/np.pi)*(eta/dummy_rho)*(10**30)*(10**-3)/NA
	MW_mix = np.dot(x_vec,MW)
	rho_bar = rho_hat*MW_mix

	get_density.ares_tilde = ares_tilde
	get_density.Z = Z
	get_density.mu_hc = mu_hc
	get_density.mu_disp = mu_disp
	get_density.mu_assoc = mu_assoc
	get_density.hres = (hresRT)*R*T
	get_density.rho = rho

	return rho_bar

def get_fugacity(T, P, MW, m, sigma, eps_k, kappa_AB, epsAB_k, Nassoc, x_vec, eta_guess):
	dens = get_density(T, P, MW, m, sigma, eps_k, kappa_AB, epsAB_k, Nassoc, x_vec, eta_guess)

	Z = get_density.Z
	mu_hc = get_density.mu_hc
	mu_disp = get_density.mu_disp
	mu_assoc = get_density.mu_assoc

	fug = np.zeros(len(x_vec))
